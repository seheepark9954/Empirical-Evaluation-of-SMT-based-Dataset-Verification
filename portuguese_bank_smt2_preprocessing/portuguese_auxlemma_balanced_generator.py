# gen_global_balanced_auxlemma.py
# Clean auxiliary-lemma encoding for global balancedness
#
# ASSUMPTIONS about input file:
# - Contains (declare-const O (Array Int Int))
# - Contains (define-fun m () Int <m>)
# - Contains label assertions: (assert (= (select O i) ...))
...
# For Portuguese / bank-full dataset
LABELS = [0, 1]
l = 2
B = 2
...
def emit_auxlemma_block(m: int) -> str:
    threshold = m // (l * B)  # constant threshold
    ...
    buf.append("(declare-const c0 Int)")
    buf.append("(declare-const c1 Int)\n")

    buf.append(sum_def(0, "sum0"))
    buf.append("")
    buf.append(sum_def(1, "sum1"))
    buf.append("")

    buf.append("(assert (= c0 sum0))")
    buf.append("(assert (= c1 sum1))\n")

    buf.append("(assert (and (<= 0 c0) (<= c0 m)))")
    buf.append("(assert (and (<= 0 c1) (<= c1 m)))\n")

    # Lemma: labels are in {0,1}
    buf.append("(assert")
    buf.append("  (and")
    for i in range(m):
        buf.append(f"    (or (= (select O {i}) 0) (= (select O {i}) 1))")
    buf.append("  )")
    buf.append(")\n")

    buf.append("(assert (= (+ c0 c1) m))\n")

    buf.append("(assert (and (>= c0 threshold) (>= c1 threshold)))")

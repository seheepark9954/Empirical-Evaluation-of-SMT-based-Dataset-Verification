...
(define-fun A () Bool
  (not
    (exists ((i Int))
      (and (>= i 0)
           (< i m)
           (or (< (select (select D i) 0) min)
               (> (select (select D i) 0) max)))
    )
  )
)

; ====== recursive property as Bool ======
(define-fun-rec minmax-ok ((i Int)) Bool
  (ite (< i 0)
       true
       (and (>= (select (select D i) 0) min)
            (<= (select (select D i) 0) max)
            (minmax-ok (- i 1))))
)
...
;"find a line(a result of each data point), which is not (A = B)"
;"If there's one output from thoese specificaions, this returns sat, because this found each value of two diff. spec.s . "
